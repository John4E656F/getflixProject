  ...
  return (
    <>
      <Slider className='wrapper' {...settings}>
        {error ? (
          <div> Some Nice Ui saying that we cannot load </div>
        ) : (
          shuffledAnime.slice(0, `${props.cardNum}`).map((anime) => (
            <React.Fragment key={anime.id}>
              <Card id={anime._id} picture={anime.picture} title={anime.title} genre={anime.genre} trailer={anime.trailer} />
            </React.Fragment>
          ))
        )}
      </Slider>
